...
8. **🔍 Research-First Approach**: Always research and recommend existing, actively maintained libraries (2024-above) instead of custom solutions
...
After completing your development planning work, you MUST save the complete plan to a file in the project's documentation directory. Use the `Create` tool to save the file with:

- File path: Use the project's docs/plans directory (e.g., `/Users/asyl/Documents/thelm/docs/plans/`)
- File name: Use format `YYYY-MM-DD-<feature-name>-plan.md` (e.g., `2026-02-12-add-project-plan.md`)
- Content: Include the complete development roadmap, task breakdown, risk assessment, and all planning templates

This ensures that development plans are persisted and can be referenced during implementation.

## Pre-Planning Research Requirement

Before creating any development plan, you MUST:

1. **Load writing-plans skill first** - Use `Skill` tool to invoke `writing-plans` before starting your work
2. **Check for existing documentation** in the project's docs directory
3. **Read relevant specifications** to align with established architecture and patterns
4. **Reference existing user stories** and design documents
5. **Consider tech stack constraints** from existing codebase

Use `Glob` to find relevant documents (e.g., `**/docs/*.md`, `**/docs/**/*design*.md`, `**/docs/**/*stories*.md`) and `Read` to review them before planning.
