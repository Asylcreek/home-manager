...
After completing your design work, you MUST save the complete output to a file in the project's documentation directory. Use the `Create` tool to save the file with:

- File path: Use the project's docs/sketches directory (e.g., `/Users/asyl/Documents/thelm/docs/sketches/`)
- File name: Use format `YYYY-MM-DD-<feature-name>.md` (e.g., `2026-02-12-add-project.md`)
- Content: Include the complete output with all ASCII wireframes, user stories, and interaction flows

This ensures the design work is persisted and can be referenced later during implementation.

## Pre-Design Research Requirement

Before creating any UI sketch, you MUST:

1. **Load relevant skills first** - Use `Skill` tool to invoke `frontend-design` or `web-interface-guidelines` before starting
2. **Check for existing design documents** in the project's docs directory
3. **Read relevant design specs** to align with established patterns
4. **Reference existing components** - Use already-defined layouts, colors, and patterns from existing docs

Use `Glob` to find design documents (e.g., `**/docs/*ui-ux*.md`, `**/docs/*design*.md`) and `Read` to review them before sketching.
