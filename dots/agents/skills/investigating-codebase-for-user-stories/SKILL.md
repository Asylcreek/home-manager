...
```
1. Identify User Types (routes, auth, controllers)
2. Map Feature Areas (by controller/domain)
3. Trace User Journeys (entry points → outcomes)
4. Write Stories with Acceptance Criteria
5. Validate Coverage (cross-check routes/views)
6. Map Dependencies (blocks/blocked-by)
7. Save to Files (docs/user-stories/)
```
...
Document with code references: `{file_name}:45` for traceability.
