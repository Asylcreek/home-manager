...
Code examples for the rules in `SKILL.md`. Apply them where they reduce a concrete TypeScript risk rather than as universal style requirements.

## Branded types

Brand primitives when same-shaped values can realistically be mixed up across a domain boundary. Validate at creation; downstream code trusts the type.
...
Follow the repository's existing nominal-type convention when it has one.
...
Let TypeScript infer local variables and function return types when the implementation already makes the type clear. Redundant annotations add noise and can drift from the value they describe.

```ts
// Prefer inference for clear implementation details.
const retryLimit = 3;
...
Add an annotation when it establishes a contract or makes TypeScript check something inference alone would not protect:

```ts
// An exported boundary promises this result shape.
export function loadUser(id: UserId): Promise<User | undefined> {
  return repository.findById(id);
}

// The annotation prevents an overly narrow inferred type.
let status: "idle" | "running" = "idle";
```

Explicit return types are also useful for recursive functions, overload implementations, declaration-facing APIs, and cases where they produce materially better errors. Do not annotate merely to repeat what TypeScript already knows.

## Minimize `as` casts

Every unchecked `as` can hide a mismatch. Prefer narrowing or validation, and isolate casts that are unavoidable at framework boundaries.
...
```ts
// Positional arguments become unclear when several values form one concept.
openFile(uri, {
  startLineNumber: 10,
  startColumn: 1,
  endLineNumber: 10,
